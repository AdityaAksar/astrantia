<div>
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
        <div class="text-center mb-8">
            <h2 class="text-4xl font-Ovo font-bold text-gray-800 mb-4">MATERI KULIAH</h2>
            <p class="text-gray-500">Download bahan ajar dan modul perkuliahan</p>
        </div>
    ...
    <div class="flex flex-wrap justify-center items-stretch gap-6">
        @forelse($materials as $material)
            <div class="w-full md:w-[350px] bg-white rounded-2xl p-6 shadow-sm hover:shadow-md border border-gray-100 transition duration-300 flex flex-col animate-fade-in-up">
                ...
                <div class="mb-6 flex-1">
                    ...
                    <h2 class="text-xl font-bold text-gray-800 leading-tight mb-2">{{ $material->title }}</h2>
                </div>
                    ...
                    <div class="space-y-2">
                        @if(is_array($material->file_path))
                            @foreach($material->file_path as $path)
                                <a href="{{ asset('storage/' . $path) }}" download target="_blank" class="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 group transition">
                                    ...
                                    <span class="text-sm font-medium text-gray-600 group-hover:text-indigo-600 truncate">Download File {{ $loop->iteration }}</span>
                                </a>
                            @endforeach
                        @elseif(is_string($material->file_path))
                            <a href="{{ asset('storage/' . $material->file_path) }}" download target="_blank" class="flex items-center gap-3 p-2 rounded-lg hover:bg-gray-50 group transition">
                                <div class="bg-red-100 p-1.5 rounded text-red-600">
                                    <svg xmlns="http://www.w3.org/2000/svg" viewBox="0 0 24 24" fill="currentColor" class="w-4 h-4"><path fill-rule="evenodd" d="M5.625 1.5H9a3.75 3.75 0 013.75 3.75v1.875c0 1.036.84 1.875 1.875 1.875H16.5a3.75 3.75 0 013.75 3.75v7.875c0 1.035-.84 1.875-1.875 1.875H5.625a1.875 1.875 0 01-1.875-1.875V3.375c0-1.036.84-1.875 1.875-1.875zM12.75 12a.75.75 0 00-1.5 0v2.25H9a.75.75 0 000 1.5h2.25V18a.75.75 0 001.5 0v-2.25H15a.75.75 0 000-1.5h-2.25V12z" clip-rule="evenodd" /></svg>
                                </div>
                                <span class="text-sm font-medium text-gray-600 group-hover:text-indigo-600 truncate">Download Dokumen</span>
                            </a>
                        @endif
                    </div>
                </div>
